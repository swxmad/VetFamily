...
const PatientCard = ({ patient, onView }) => {  
  ...
  return (
    <div 
      className={`${styles.patientCard} ${patient.status === 'completed' ? styles.completed : ''} ${styles.clickable}`}
      onClick={handleCardClick}
    >
      <div className={styles.cardHeader}>
        <h3 className={styles.patientName}>{truncate(patient.name)}</h3>
        <span className={`${styles.status} ${patient.status === 'active' ? styles.active : styles.completed}`}>
          {patient.status === 'active' ? 'Активный' : 'Завершён'}
        </span>
      </div>
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.infoRow}>
          ...
          <span className={styles.value}>{truncate(formatPhone(patient.ownerPhone))}</span>
        </div>
        <div className={styles.infoRow}>
          <span className={styles.label}>Врач:</span>
          <span className={styles.value}>{truncate(patient.doctor?.fullName || 'Не назначен')}</span>
        </div>
      </div>
